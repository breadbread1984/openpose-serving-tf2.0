...
class Predictor(object):
    ...
    def predict(self, oriImg):
        ...
        for m in range(len(multiplier)):
            ...
            # send request
            data = json.dumps({"signature_name": "serving_default", "instances": [{"input_1": np.squeeze(input_img).tolist()}]});
            json_response = requests.post("http://localhost:8503/v1/models/openpose:predict", data = data, headers = self.headers);
            output_blobs = (
                np.array(json.loads(json_response.text)["predictions"][0]['Mconv7_stage6_L1']),
                np.array(json.loads(json_response.text)["predictions"][0]['Mconv7_stage6_L2'])
            );
...
if __name__ == "__main__":
    ...
    canvas = predictor.visualize(img, all_peaks, candidate, subset);
